...
<aside
    ...
    class="fixed lg:static w-64 inset-y-0 left-0 z-30 flex flex-col bg-sidebar flex-shrink-0
           transform transition-all duration-300 ease-in-out overflow-hidden"
    x-show="sidebarOpen || window.innerWidth >= 1024"
    style="display: flex;">

    {{-- Logo --}}
    <div class="flex items-center h-16 border-b border-slate-700 flex-shrink-0 overflow-hidden transition-all duration-300"
        ...
        <div class="flex items-center gap-3 min-w-0">
            ...
            <span x-show="!sidebarCollapsed"
                  ...
                  x-transition:leave-start="opacity-100"
                  x-transition:leave-end="opacity-0"
                  class="text-white font-bold text-[15px] leading-tight tracking-tight whitespace-nowrap">
    ...
    {{-- Navegação --}}
    <nav class="flex-1 overflow-y-auto py-3 transition-all duration-300"
         :class="sidebarCollapsed ? 'px-1.5 space-y-1' : 'px-2 space-y-1'">
        @foreach($menuGroups as $group => $items)
            <div x-data="{ open: true }">
                ...
                <div class="h-px bg-slate-700/40 mx-1 my-2" x-show="sidebarCollapsed" style="display:none;"></div>
                ...
                <div x-show="open || sidebarCollapsed"
                    ...
                    @foreach($items as $item)
                        ...
                        <a href="{{ $url }}"
                           ...
                           :class="sidebarCollapsed ? 'justify-center p-3 w-full' : 'gap-3 px-3 py-2'">
                            ...
                            <span x-show="!sidebarCollapsed"
                                  ...
                                  x-transition:leave-start="opacity-100"
                                  x-transition:leave-end="opacity-0"
                                  class="flex-1 truncate leading-none">
                                {{ __($item->label) }}
                            </span>

                            @if(!$hasRoute)
                                <span class="text-[10px] text-slate-500 font-normal" x-show="!sidebarCollapsed">
                                    {{ __('em breve') }}
                                </span>
                            @endif
                        </a>
    ...
    <div class="flex-shrink-0 border-t border-slate-700 hidden lg:flex">
        <button
            @click="sidebarCollapsed = !sidebarCollapsed"
            class="flex items-center w-full py-3 text-slate-400
                   hover:text-slate-200 hover:bg-slate-700/40 transition-colors duration-150"
    ...
    {{-- Usuário logado --}}
    <div class="flex-shrink-0 border-t border-slate-700 transition-all duration-300"
         :class="sidebarCollapsed ? 'p-1.5' : 'p-3'">
        <div class="flex items-center rounded-lg hover:bg-slate-700/50 transition-colors cursor-default"
