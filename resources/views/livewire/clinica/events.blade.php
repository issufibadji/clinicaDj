...
<div>
    <div class="mb-6 flex items-center justify-between">
        <div>
            <h1 class="text-xl font-bold text-slate-800 dark:text-slate-100">{{ __('Eventos') }}</h1>
            <p class="text-sm text-slate-500 dark:text-slate-400 mt-0.5">{{ __('Agenda de eventos e compromissos.') }}</p>
        </div>
        @can('create', \App\Models\Event::class)
            <button wire:click="openCreate" class="btn-primary flex items-center gap-2 px-4 py-2 text-sm">
                <x-heroicon-o-plus class="w-4 h-4" />{{ __('Novo evento') }}
            </button>
        @endcan
    </div>
    ...
    <div class="card mb-4">
        <input wire:model.live.debounce.300ms="search" type="text" placeholder="{{ __('Buscar evento...') }}" class="input" />
    </div>
        ...
        <table class="w-full text-sm">
            <thead>
                <tr class="border-b border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/60">
                    <th class="px-5 py-3 text-left text-xs font-semibold text-slate-500 uppercase tracking-wider">{{ __('Evento') }}</th>
                    <th class="px-5 py-3 text-left text-xs font-semibold text-slate-500 uppercase tracking-wider">{{ __('Início') }}</th>
                    <th class="px-5 py-3 text-left text-xs font-semibold text-slate-500 uppercase tracking-wider">{{ __('Término') }}</th>
                    <th class="px-5 py-3 text-left text-xs font-semibold text-slate-500 uppercase tracking-wider">{{ __('Criado por') }}</th>
                    <th class="px-5 py-3 text-left text-xs font-semibold text-slate-500 uppercase tracking-wider">{{ __('Visib.') }}</th>
            ...
            <tbody class="divide-y divide-slate-100 dark:divide-slate-700/60">
                ...
                @empty
                    <tr><td colspan="6" class="px-5 py-12 text-center text-sm text-slate-400">{{ __('Nenhum evento encontrado.') }}</td></tr>
                @endforelse
            </tbody>
        </table>
        @if($events->hasPages())
            ...
            <div class="relative w-full max-w-lg bg-white dark:bg-slate-800 rounded-2xl shadow-2xl border border-slate-200 dark:border-slate-700 max-h-[90vh] flex flex-col">
                <div class="flex items-center justify-between px-6 py-4 border-b border-slate-100 dark:border-slate-700 flex-shrink-0">
                    <h2 class="text-base font-semibold text-slate-800 dark:text-slate-100">
                        {{ $editingId ? __('Editar evento') : __('Novo evento') }}
                    </h2>
                ...
                <div class="overflow-y-auto flex-1 p-6 space-y-4">
                    <div>
                        <label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1.5">{{ __('Título') }}</label>
                        <input wire:model="formTitle" type="text" class="input" />
                        @error('formTitle')<p class="mt-1 text-xs text-red-600">{{ $message }}</p>@enderror
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1.5">{{ __('Descrição') }}</label>
                        <textarea wire:model="formDescription" rows="2" class="input resize-none"></textarea>
                    </div>
                    <div class="grid grid-cols-2 gap-3">
                        <div>
                            <label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1.5">{{ __('Início') }}</label>
                        ...
                        <div>
                            <label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1.5">{{ __('Término') }}</label>
                    ...
                    <div>
                        <label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">{{ __('Cor') }}</label>
                    ...
                    <label class="flex items-center gap-3 cursor-pointer">
                        ...
                        <span class="text-sm text-slate-700 dark:text-slate-300">{{ __('Visível para todos') }}</span>
                    </label>
                </div>
